...
# ============================================================================
# WORKSPACE RUNTIME (ENGINE-AGNOSTIC)
# ============================================================================

class Workspace:
    """
    Workspace runtime-safe.
    ...
    - espone paths
    - conserva metadata
    - orchestra capabilities esterne
    """
    ...
    def __init__(
        ...
        *,
        ...
        logger.debug(
            "WorkspaceRuntime created id=%s name=%s path=%s",
            self.id,
            self.name,
            self.base_path,
        )
    ...
    async def initialize(self) -> None:
        if self.state not in {WorkspaceState.CREATED}:
            return
    ...
    # ------------------------------------------------------------------
    # CAPABILITIES (CORE IDEA)
    # ------------------------------------------------------------------

    def register_capability(self, name: str, obj: Any) -> None:
        self._capabilities[name] = obj
        logger.debug("Workspace %s capability registered: %s", self.id, name)
    ...
    def __repr__(self) -> str:
        return f"Workspace(id={self.id}, state={self.state.value})"
